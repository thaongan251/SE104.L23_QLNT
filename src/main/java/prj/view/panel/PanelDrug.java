...
import prj.view.dialog.AddDrugFrm;
...
public class PanelDrug extends javax.swing.JPanel {
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addComponent(pnButton, javax.swing.GroupLayout.DEFAULT_SIZE, 929, Short.MAX_VALUE))
                .addGap(30, 30, 30))
        );
            ...
            if (confimed == 0) {//YES_OPTION
                for (int i = selectedMultiRows.length - 1; i >= 0; i--) {
                    DrugController.getInstance().deleteFromDatabase(
                            DrugController.getInstance().getList().get(selectedMultiRows[i])
                    );

                    DrugController.getInstance().getList().remove(selectedMultiRows[i]);
    ...
    private void JDBC() {
        ...
        try {
            ...
            while (rs.next()) {
                String MaThuoc = rs.getString("MATHUOC").trim();
                String TenThuoc = rs.getString("TENTHUOC").trim();
                String PhanNhom = rs.getString("PHANNHOM").trim();
                String PhanLoai = rs.getString("PHANLOAI").trim();
                String ThanhPhan = rs.getString("THANHPHAN").trim();
                String DVT = rs.getString("DVT").trim();
