...
import prj.view.dialog.AddImportVoucherFrm;
...
public class PanelImportVoucher extends javax.swing.JPanel {
    ...
    private void setIcon() {
        ...
        lbLogoView.setIcon(ImageUtilities.getViewIcon());
    }
            ...
            if (confimed == 0) {//YES_OPTION
                for (int i = selectedMultiRows.length - 1; i >= 0; i--) {
                    ImportVoucherController.getInstance().deleteFromDatabase(
                            ImportVoucherController.getInstance().getList().get(selectedMultiRows[i])
                    );

                    ImportVoucherController.getInstance().getList().remove(selectedMultiRows[i]);
    ...
    private void btnEditMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnEditMouseClicked
        ...
        if (selectedIndex == -1) {
            ...
            AddImportVoucherFrm addImportFrm = new AddImportVoucherFrm(selectedIndex, true);
            addImportFrm.setVisible(true);
    ...
    private void btnViewMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnViewMouseClicked
        ...
        if (selectedIndex == -1) {
            ...
            AddImportVoucherFrm addImportFrm = new AddImportVoucherFrm(selectedIndex, false);
            addImportFrm.setVisible(true);
        }
    ...
    private void btnViewMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnViewMouseEntered
        ...
        btnView.setToolTipText("Xem chi tiết phiếu nhập");
    ...
    private void JDBC() {
        ...
        try {
            ...
            while (rs.next()) {
                String maphieunhap = rs.getString("MAPN").trim();
                String manv = rs.getString("MANV").trim();
                String tennv = rs.getString("TENNV").trim();
                String mancc = rs.getString("MANCC").trim();
                String tenncc = rs.getString("TENNCC").trim();
                LocalDate ngaymua = rs.getDate("NGAYMUA").toLocalDate();
                float tongTien = rs.getFloat("TRIGIA");
                ImportVoucher iv = new ImportVoucher(maphieunhap, manv, tennv, mancc, tenncc, ngaymua, tongTien);
                list.add(iv);
                tblList.setModel(ImportVoucherController.getInstance().toTable());
            }

            ResultSet rsDetail = st.executeQuery("SELECT * FROM CTPN");
            while (rsDetail.next()) {
                String mactpn = rsDetail.getString("MACTPN").trim();
                String maPn = rsDetail.getString("MAPN").trim();
                String mathuoc = rsDetail.getString("MATHUOC").trim();
                int sl = rsDetail.getInt("SL");
                float dongia = rsDetail.getFloat("DONGIA");
                float thanhtien = rsDetail.getFloat("THANHTIEN");
                ImportVoucherDetail detail = new ImportVoucherDetail(
                        mactpn, maPn, mathuoc, sl, dongia, thanhtien
                );

                for (int i = 0; i < list.size(); i++) {
                    if (maPn.equals(list.get(i).getsMaPN())) {
                        list.get(i).getDetailList().add(detail);
                    }
                }
            }
