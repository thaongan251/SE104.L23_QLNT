...
import prj.view.dialog.AddReceiptFrm;
...
public class PanelReceipt extends javax.swing.JPanel {
            ...
            if (confimed == 0) {//YES_OPTION
                for (int i = selectedMultiRows.length - 1; i >= 0; i--) {
                    ReceiptController.getInstance().deleteFromDatabase(
                            ReceiptController.getInstance().getList().get(selectedMultiRows[i])
                    );

                    ReceiptController.getInstance().getList().remove(selectedMultiRows[i]);
    ...
    private void btnViewMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnViewMouseEntered
        ...
        btnView.setToolTipText("Xem chi tiết hoá đơn");
    ...
    public void JDBC() {
        ...
        try {
            ...
            ResultSet rsDetail = st.executeQuery("SELECT * FROM CTHD");
            while (rsDetail.next()) {
                String macthd = rsDetail.getString("MACTHD").trim();
                String maHd = rsDetail.getString("MAHD").trim();
                ...
                ReceiptDetail detail = new ReceiptDetail(
                        macthd, maHd, mathuoc, sl, dongia, thanhtien
                );

                for (int i = 0; i < list.size(); i++) {
                    if (maHd.equals(list.get(i).getsMaHD())) {
